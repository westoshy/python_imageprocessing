...
cv2.circle(mask, center=(cx, cy), color=255, radius=30, thickness=-1)
...
cv2.imshow("low pass", magnitude.astype(np.uint8))
cv2.imshow("filtered", np.uint8(output))
cv2.waitKey(-1)


cv2.imwrite("output/lowpass.bmp", magnitude.astype(np.uint8))
